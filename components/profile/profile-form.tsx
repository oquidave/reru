...
export function ProfileForm({ client, locations, currentEmail, phone }: ProfileFormProps) {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      other_location: client.other_location ?? '',
      plan:           client.plan ?? 'monthly',
      collection_day: client.collection_day ?? 'Wednesday',
      address:        client.address ?? '',
      landmark:       client.landmark ?? '',
      property_type:  client.property_type ?? 'household',
      bin_count:      client.bin_count ?? 1,
      alt_phone:      client.alt_phone ?? '',
      alt_phone_is_whatsapp: client.alt_phone_is_whatsapp ?? false,
      email:          currentEmail,
      password:       '',
    },
  })
